...
export default function PaymentResult() {
  ...
  useEffect(() => {
    // Auto redirect về profile sau 5s nếu thanh toán thành công
    if (status === "success") {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 text-center animate-in zoom-in-95 duration-300">
        ...
        <p className="text-slate-600 mb-6 text-base leading-relaxed">
          {isSuccess
            ? "Gói nâng cấp của bạn đã được kích hoạt thành công. Bạn có thể bắt đầu sử dụng các tính năng Premium ngay bây giờ!"
            : isFailed
        ...
        {isSuccess && (
          <div className="flex items-center justify-center gap-2 text-sm text-slate-500 mb-6 bg-emerald-50 py-3 rounded-lg">
            <Clock size={16} className="animate-pulse" />
            <span className="font-medium">
              Tự động chuyển về trang cá nhân sau{" "}
        ...
        <div className="space-y-3">
          ...
          >
            <span className="flex items-center justify-center gap-2">
              {isSuccess ? "Về trang cá nhân" : "Về trang cá nhân"}
              <ArrowRight size={18} />
            </span>
          </button>
