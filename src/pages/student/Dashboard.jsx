...
export default function StudentDashboard() {
  return (
    <div className="space-y-8">
      {/* 1. Hero Banner */}
      <div className="bg-gradient-to-r from-emerald-500 to-green-500 rounded-2xl p-8 text-white shadow-lg shadow-emerald-200 flex items-center justify-between relative overflow-hidden">
        <div className="relative z-10 max-w-lg">
          <div className="flex items-center gap-2 mb-2 text-emerald-100 text-sm font-medium">
            <span className="bg-white/20 px-2 py-1 rounded">N5 Beginner</span>
            <span>Bài đang học dở</span>
          </div>
          <h2 className="text-3xl font-bold mb-4">
            Bài 3: Cách chào hỏi và giới thiệu bản thân
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: "Số giờ đã học", val: "12.5h", color: "bg-emerald-50 text-emerald-600" },
          { label: "Bài tập đã làm", val: "8/10", color: "bg-lime-50 text-lime-600" },
          { label: "Điểm trung bình", val: "9.2", color: "bg-green-50 text-green-600" },
        ].map((stat, idx) => (
          <div
            key={idx}
          ...
          >
            ...
            <p
              className={`text-3xl font-bold mt-2 w-fit px-2 rounded ${stat.color}`}
            >
              {stat.val}
            </p>
          </div>
          ...
          {[1, 2, 3].map((item) => (
            ...
            >
              <div className="h-40 bg-slate-200 relative">
                <img
                  src={`https://placehold.co/600x400/e2e8f0/1e293b?text=Course+${item}`}
                  alt="Course"
              ...
              <div className="p-4">
                ...
                <h4 className="font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">
                  Luyện thi JLPT N4 cấp tốc - Phần ngữ pháp
                </h4>
