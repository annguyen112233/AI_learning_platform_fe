    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
           ...
           <p className="text-slate-500">Tiếp tục hành trình chinh phục tiếng Nhật nào!</p>
        </div>
        <div className="flex items-center gap-3">
            <div className="w-64">
                <Input icon={Search} placeholder="Tìm khóa học..." />
            </div>
            <Button variant="outline" className="h-[42px]"><Filter size={18} /> Lọc</Button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {courses.map((course) => (
          <div key={course.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-lg transition-all group flex flex-col">
            {/* Image */}
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <div className="mt-auto">
                ...
                >
                   {course.progress === 0 ? "Bắt đầu học" : course.progress === 100 ? "Ôn tập lại" : "Tiếp tục học"}
                </Button>
              </div>
            </div>
          </div>
